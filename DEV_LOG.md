...
## Session 41 - Bug Fix Pass & Survival Rebalance (2026-03-11)

Fixed 7 bugs and rebalanced hunger system. Released as v1.1.2.

### Bug Fixes
- **Explorer's Journal respawn**: `explorers_journal_pickup.gd` `interact()` never set `ChunkManager.sinkhole_book_collected = true`, so the journal respawned every time the sinkhole chunk reloaded. Added `_find_chunk_manager()` helper and flag set on collection.
- **Rock spire collision**: The stone cairn landmark near the sinkhole used `Node3D` root with no collision. Changed to `StaticBody3D` with a `CollisionShape3D` (3x10x3 box).
- **Smithing auto-smelt**: Smithing station automatically withdrew ore from player inventory on smelt completion. Removed `_try_auto_smelt()` so player must manually place ore, matching drying rack behavior.
- **Weather forecast frozen**: `WeatherManager` only reset `_rolled_today` during Night period. Sleeping from Evening/Dusk (hour >= 19) skipped Night, so the flag stayed true and weather never rolled. Connected to `day_changed` signal to reset on any new day.
- **Weather forecast disconnected**: `_daily_weather_roll()` did an independent random roll (67% clear) instead of applying the forecasted `next_weather`. Rewrote to apply forecast as actual weather, then generate new forecast for tomorrow.
- **Controller frozen on game start**: `TouchControls.menu_open` (static var) persisted as `true` across scene reloads, blocking all controller input. Reset to `false` in player `_ready()`.
- **Save migration for hunger rate**: Old saves stored outdated hunger rate values (0.05/0.04) that overrode new defaults. Added migration in `save_load.gd` to detect and upgrade old rates.

### Survival Rebalance
- **Hunger scales with day length**: Hunger depletion, starvation damage, and health regen now scale by `BASE_DAY_LENGTH_MINUTES / day_length_minutes`. A 1-minute day depletes hunger 20x faster than a 20-minute day.
- **Hunger rate increased**: Base rate changed from 0.05 to 0.167 so hunger depletes fully in half a game-day (empty by noon without eating = twice per day). Explorer's Journal bonus rate updated from 0.04 to 0.133 (still 20% slower).

### Release
- Exported macOS (notarized), Windows, and iOS builds
- Created GitHub release v1.1.2

### Files Modified
| File | Changes |
|------|---------|
| `scripts/world/explorers_journal_pickup.gd` | Set `sinkhole_book_collected` on collection, added `_find_chunk_manager()` |
| `scripts/world/chunk_manager.gd` | Rock spire: `Node3D` → `StaticBody3D` with collision |
| `scripts/campsite/structure_smithing_station.gd` | Removed `_try_auto_smelt()` and auto-start on completion |
| `scripts/world/weather_manager.gd` | Connected `day_changed` signal, apply `next_weather` at dawn |
| `scripts/player/player_stats.gd` | Day-length hunger scaling, rate 0.05 → 0.167 |
| `scripts/player/player_controller.gd` | Reset `TouchControls.menu_open` in `_ready()`, journal rate 0.04 → 0.133 |
| `scripts/core/save_load.gd` | Migrate old hunger rates on load |
| `tests/test_bug_regressions.gd` | Added 4 regression tests |

---

## Session 42 - iOS Bug Fixes: Journal Touch Controls, Eat Button, Crouch Removal (2026-03-13)

Fixed 3 iPad-specific issues discovered during play-testing.

### Bug Fixes

- **Explorer's Journal stuck on iPad**: Journal had no touch controls for page turning or dismissal — player got completely stuck on first page. Added swipe left/right gesture support (80px threshold), added tap-friendly `<` / `>` arrow buttons on left/right edges of the book, and updated hint text to say "Swipe or tap arrows to turn pages" on iOS. Close button (X) was already present.

- **Eat button disappearing with food in stock**: When any menu closed, `_process()` restored core button visibility but skipped context buttons (eat, use, crouch). The eat button stayed hidden until the next `inventory_changed` signal fired (i.e., picking up new food). Added `has_consumable()` check to the menu-close path so eat button visibility is immediately restored.

- **Crouch button removed from iOS**: Crouch added UI complexity with little gameplay value on touch screens. Removed the crouch button creation entirely and removed the per-frame `is_near_cliff_edge()` check from `_process()`, saving processing power.

### Files Modified
| File | Changes |
|------|---------|
| `scripts/ui/journal_ui.gd` | Added swipe gesture tracking, mobile prev/next arrow buttons, iOS hint text |
| `scripts/ui/touch_controls.gd` | Fixed eat button restore on menu close, removed crouch button and per-frame cliff check |

---

## Session 43 - iOS Bug Fix: Touch Buttons Dead After Loading Save (2026-03-13)

Fixed a bug where BAG, CRAFT, and MENU touch buttons became unresponsive on iOS after loading a saved game that required a scene reload (world seed mismatch).

### Bug Fix

- **Touch buttons dead after save load**: When loading a save with a different world seed, `reload_current_scene()` is called. But TouchControls is added to `get_tree().root` (not the main scene), so it survives the reload. The new HUD then creates a second TouchControls instance — duplicate buttons at the same position cause actions to fire twice, toggling menus ON then OFF instantly. Also, the pause menu left `get_tree().paused = true` and the reload path never unpaused.

  **Fix**: HUD now removes any existing TouchControls from root before creating a new one. SaveLoad now unpauses the tree before calling `reload_current_scene()`.

### Files Modified
| File | Changes |
|------|---------|
| `scripts/ui/hud.gd` | Remove existing TouchControls from root before creating new instance |
| `scripts/core/save_load.gd` | Unpause tree before `reload_current_scene()` |
| `tests/test_touch_controls.gd` | Added regression test verifying both fixes |

---

---

## Session 44 - Controller Prompt Fix & Test Suite Cleanup (2026-03-13)

Fixed a controller prompt bug and performed major test suite maintenance — removing dead tests and fixing fragile ones.

### Bug Fix

- **Controller unequip prompt wrong**: `CONTROLLER_PROMPTS["unequip"]` was `"□"` (square) instead of `"✕"` (cross/circle). Fixed in `input_manager.gd`.

### Test Suite Cleanup

**Removed 5 dead tests** for code that was refactored:
- `_complete_drying/smoking/smelting` no longer use `player_inventory`
- Kitchen `interact()` cooks food, doesn't restore hunger directly
- River rendering uses grid quads, no miter logic exists

**Fixed 12 fragile string-matching tests**:
- Rewrote `player_stats` zero-guard test as behavioral (instantiates PlayerStats, tests actual `get_health/hunger_percent` with zero max)
- Switched `load()` to `FileAccess.open()` for scripts with autoload dependencies (player_controller, drying_rack, smoker, bark_map_ui)
- Fixed search windows too narrow to find patterns (death_closes_menus, notification_timer, health_hunger_clamped)
- Fixed `_try_eat` test finding wrong occurrence of HEALING_ITEMS loop
- Fixed `use_equipped` test searching wrong direction for `is_placing` guard
- Updated `processing_structures` test for `pending_output` refactor

All 1150 tests pass (0 failures).

### Files Modified
| File | Changes |
|------|---------|
| `scripts/systems/input_manager.gd` | Fixed unequip controller prompt from "□" to "✕" |
| `tests/test_bug_regressions.gd` | Removed 5 dead tests, fixed 12 fragile tests |

---

## Session 45 - iOS Bug Fix: Touch Menu Buttons After Save Load (2026-03-15)

Fixed a follow-up bug where BAG, CRAFT, and MENU touch buttons still didn't work after loading a saved game, even with the Session 43 duplicate-instance fix in place.

### Bug Fix

- **Touch menu buttons not opening after load**: After `reload_current_scene()`, `Input.parse_input_event()` failed to route actions to the new scene's menu nodes. Bypassed the input system entirely for BAG/CRAFT/MENU buttons — directly find menu nodes via group lookups and call their toggle methods. Also added `pause_menu` to the `"pause_menu"` group so the lookup works for all three menus.

### Files Modified
| File | Changes |
|------|---------|
| `scripts/ui/touch_controls.gd` | Direct group-lookup toggle for BAG/CRAFT/MENU instead of Input.parse_input_event() |
| `scripts/ui/pause_menu.gd` | Added self to "pause_menu" group in _ready() |

---

## Session 46 - iOS v1.1.0 App Store Update (2026-03-16)

Prepared iOS App Store update with all bug fixes since v1.0.0 submission. Added Godot UID tracking files.
...
| File | Changes |
|------|---------|
| `export_presets.cfg` | iOS version 1.0.0 → 1.1.0 |
| `scripts/ui/hint_manager.gd.uid` | Added (Godot UID tracking) |
| `scripts/ui/touch_controls.gd.uid` | Added (Godot UID tracking) |
...
## Session 52 - Fix Pond Edge Terrain Fall-Through (2026-04-12)

Player could swim toward shore at the campsite pond and clip through the terrain, falling into the void. This was a long-standing issue (related to Sessions 21, 53, 54 which addressed HeightMapShape3D fall-through before the BoxShape3D system was introduced).

### Root Cause

In `terrain_chunk.gd:_generate_box_collision()`, the collision box for each cell had its **top at the cell height** and extended **downward by only `max(height, 0.5)`** (for positive heights) or **0.5 units upward** (for negative heights). At the pond-to-land boundary, the pond floor cell (height=-2.5) had a box top at -2.5, while the adjacent shore cell (height=+0.486) had a box bottom at -0.014. This left a **2.486-unit vertical gap** between adjacent cells that the player could swim through horizontally.

Additionally, negative-height cells used `height + box_height / 2.0` which placed thin slabs extending upward from the cell height, creating additional gaps between ramp cells at different depths.

Note: Initial fix attempts targeted `terrain_generator.gd`, which turned out to be **dead code** — not referenced by any scene. All runtime terrain is generated by the chunk system (`chunk_manager.gd` + `terrain_chunk.gd`).

### Fix

Unified the collision formula for all cells: `box_height = abs(height) + 3.0` with `box_y_center = height - box_height / 2.0`. Every box now extends 3 extra units below its surface, ensuring adjacent cells always overlap vertically — even at water-land transitions where height changes abruptly.

### Additional Improvements

- **WaterArea sizing**: Removed +1.0 oversizing from pond WaterArea X/Z dimensions in `fishing_spot.gd` to prevent premature water-exit signals
- **Water-exit grace timer**: Added 0.3s grace period in `player_controller.gd` that keeps swim physics active briefly after leaving WaterArea, preventing instant gravity at the boundary
- **Fall protection**: Updated emergency fall recovery to respect the grace timer

### Files Modified
| File | Changes |
|------|---------|
| `scripts/world/terrain_chunk.gd` | Unified collision box formula: `abs(height) + 3.0` depth for all cells (sync and batched) |
| `scripts/resources/fishing_spot.gd` | Removed +1.0 WaterArea oversizing on X/Z |
| `scripts/player/player_controller.gd` | Added water-exit grace timer (0.3s), updated fall protection |
| `tests/test_bug_regressions.gd` | Added 3 regression tests: collision depth, WaterArea sizing, grace timer |
| `DEV_LOG.md` | Removed completed website deploy task |
