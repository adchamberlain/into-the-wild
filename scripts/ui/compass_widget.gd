...
# Layout (1.5x scaled)
const RADIUS: float = 67.5
const TICK_OUTER: float = 64.5
const TICK_INNER_CARDINAL: float = 49.5
const TICK_INNER_INTER: float = 55.5
const LABEL_RADIUS: float = 40.5
const CENTER_DOT_RADIUS: float = 4.5
const CHEVRON_SIZE: float = 10.5

# Colors (with transparency)
const BG_COLOR: Color = Color(0.1, 0.1, 0.12, 0.5)
const RING_COLOR: Color = Color(0.5, 0.5, 0.5, 0.55)
const TICK_COLOR: Color = Color(0.7, 0.7, 0.7, 0.65)
const NORTH_COLOR: Color = Color(1.0, 0.85, 0.3, 0.75)
const LABEL_COLOR: Color = Color(0.9, 0.9, 0.9, 0.75)
const CENTER_COLOR: Color = Color(0.9, 0.9, 0.9, 0.65)
const CHEVRON_COLOR: Color = Color(1.0, 0.85, 0.3, 0.75)
...
func _draw() -> void:
	...
	# 2. Outer ring
	_draw_ring(center, RADIUS, RING_COLOR, 3.0)
	...
	for card: Dictionary in CARDINALS:
		...
		draw_line(inner, outer, color, 3.0)
	...
	for inter_angle: float in INTERCARDINALS:
		...
		draw_line(inner, outer, TICK_COLOR, 2.25)
	...
	for card: Dictionary in CARDINALS:
		...
		var font_size: int = 24 if card.label == "N" else 21
