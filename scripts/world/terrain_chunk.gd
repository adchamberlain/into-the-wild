...
func _add_top_face_cached(st: SurfaceTool, x: float, z: float, size: float, height: float, cx: int, cz: int) -> void:
	...
	# Triangle 1: v0, v1, v2 (CCW from above = front-facing)
	st.set_color(color0)
	...
	# Triangle 2: v0, v2, v3 (CCW from above = front-facing)
	st.set_color(color0)
	st.set_normal(normal)
	st.set_uv(uvs[0])
	st.add_vertex(v0)
...
func _add_side_quad_ao(st: SurfaceTool, v0: Vector3, v1: Vector3, v2: Vector3, v3: Vector3, normal: Vector3, ao_top: float, ao_bottom: float) -> void:
	...
	if total_height > grass_thickness:
		...
		# Triangle 1: v0, g2, v1 (CCW from outside = front-facing)
		...
		# Triangle 2: v0, g3, g2 (CCW from outside = front-facing)
		st.set_color(grass_top_col)
		st.set_normal(normal)
		st.set_uv(uv_grass_top_l)
		st.add_vertex(v0)
		...
		# Triangle 1: g3, v2, g2 (CCW from outside = front-facing)
		...
		# Triangle 2: g3, v3, v2 (CCW from outside = front-facing)
		st.set_color(dirt_top_col)
		st.set_normal(normal)
		st.set_uv(dirt_uvs[0])
		st.add_vertex(g3)
		...
		# Triangle 1: v0, v2, v1 (CCW from outside = front-facing)
		st.set_color(grass_top_col)
		st.set_normal(normal)
		...
		# Triangle 2: v0, v3, v2 (CCW from outside = front-facing)
		st.set_color(grass_top_col)
		st.set_normal(normal)
		st.set_uv(side_uvs[0])
		st.add_vertex(v0)
