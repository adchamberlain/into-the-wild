...
func interact(player: Node) -> bool:
	...
	if inventory and inventory.has_method("can_add_item") and not inventory.can_add_item("torch", 1):
		_show_notification("Can't carry more torches. Store items first.", Color(1.0, 0.5, 0.5))
		return false
